...
fit
#nn=node number, cex= graph bada kar dega , 
#
rpart.plot(fit, extra = 106, cex=.8,nn=T)  #plot

printcp(fit) #select complexity parameter
prunetree2 = prune(fit, cp=.014)#clasification of model,prune the tree to avoid
...
#green survived , blue chances of non survival
